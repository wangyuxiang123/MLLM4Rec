...
class GamesDataset(AbstractDataset):
    ...
    @classmethod
    def url(cls):
        # games_meta.json.gz from snap.stanford.edu does not contain full meta info
    ...
    def preprocess(self):
        ...
        meta_raw, meta_raw_img = self.load_meta_dict()
        ...
        meta_img_url = {smap[k]: v for k, v in meta_raw_img.items() if k in smap}
        dataset = {'train': train,
                   'val': val,
                   'test': test,
                   'meta': meta,
    ...
    def load_meta_dict(self):
        ...
        with gzip.open(file_path, 'rb') as f:
            for line in f:
                item = eval(line)
                if 'title' in item and len(item['title']) > 0 and len(item['imageURL']) > 0:

                    meta_dict[item['asin'].strip()] = item['title'].strip()
                    if len(item['imageURL']) == 1:
                        meta_dict_img[item['asin'].strip()] = item['imageURL'][0].strip()
                    else:
                        choice = random.randint(0, len(item['imageURL']) - 1)
                        meta_dict_img[item['asin'].strip()] = item['imageURL'][choice].strip()
        return meta_dict, meta_dict_img
