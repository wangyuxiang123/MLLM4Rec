...
def seq_to_token_ids(args, seq, candidates, label, text_dict, text_img_dict, tokenizer, prompter, eval=False):
    def truncate_title(title):
        title_ = tokenizer.tokenize(title)[:args.llm_max_title_len]
        title = tokenizer.convert_tokens_to_string(title_)
        return title

    seq_t = ' \n '.join(['(' + str(idx + 1) + ') {' + truncate_title(
        text_dict[item]) + ' : ' + truncate_title(text_img_dict[item])
                         for idx, item in enumerate(seq)])
    can_t = ' \n '.join(['(' + chr(ord('A') + idx) + ') {' + truncate_title(
        text_dict[item]) + ' : ' + truncate_title(text_img_dict[item])
    ...
    # print(f"seq:{seq},candidates:{candidates}")
...
class LLMDataloader():
    def __init__(self, args, dataset):
        ...
        self.text_img_dict = seq_dataset['meta_img_des']
        ...
        del self.val_probs, self.val_labels, self.val_metrics
        ...
        del retrieved_file,self.test_probs,self.test_labels,self.test_metrics
    ...
    def _get_train_dataset(self):
        dataset = LLMTrainDataset(self.args, self.train, self.max_len, self.rng,
                                  self.text_dict, self.text_img_dict, self.tokenizer, self.prompter)
        return dataset
    ...
    def _get_eval_dataset(self, mode):
        if mode == 'val':
            dataset = LLMValidDataset(self.args, self.train, self.val, self.max_len, self.rng, \
                                      self.text_dict, self.text_img_dict, self.tokenizer, self.prompter, self.val_users, \
                                      self.val_candidates)
        elif mode == 'test':
            dataset = LLMTestDataset(self.args, self.train, self.val, self.test, self.max_len, \
                                     self.rng, self.text_dict, self.text_img_dict, self.tokenizer, self.prompter, self.test_users, \
...
class LLMTrainDataset(data_utils.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return seq_to_token_ids(self.args, seq, candidates, answer, self.text_dict, self.text_img_dict, self.tokenizer, self.prompter, eval=False)


class LLMValidDataset(data_utils.Dataset):
    def __init__(self, args, u2seq, u2answer, max_len, rng, text_dict, text_img_dict, tokenizer, prompter, val_users, val_candidates):
        self.args = args
        self.u2seq = u2seq
    ...
    def __getitem__(self, index):
        ...
        return seq_to_token_ids(self.args, seq, candidates, answer, self.text_dict, self.text_img_dict, self.tokenizer, self.prompter, eval=True)


class LLMTestDataset(data_utils.Dataset):
    def __init__(self, args, u2seq, u2val, u2answer, max_len, rng, text_dict, text_img_dict, tokenizer, prompter, test_users, test_candidates):
        self.args = args
        self.u2seq = u2seq
        self.u2val = u2val
        self.u2answer = u2answer
